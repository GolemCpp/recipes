...
def script(ctx):
    ...
    command = ['cmake', mailio_dir] + opt_arch + [
        opt_variant, opt_target, '-DMAILIO_BUILD_DOCUMENTATION=OFF',
    ...
    version_file = ctx.make_project_path(
        os.path.join('include', 'version.hpp.in'))
    if os.path.exists(version_file):
        os.remove(version_file)

    mailio_include_dir = ctx.make_project_path(
        os.path.join('include', 'mailio'))

    version_file = os.path.join(target_dir, 'version.hpp')
    if os.path.exists(version_file):
        shutil.copyfile(src=version_file,
                        dst=os.path.join(mailio_include_dir, 'version.hpp'))

    export_file = os.path.join(target_dir, 'export.hpp')
    if os.path.exists(export_file):
        shutil.copyfile(src=export_file,
