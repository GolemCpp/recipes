...
- **Name a recipe at the qualification that makes it unambiguous.** Golem drops the last field of an identity until something answers, therefore a shorter name serves a source wherever it is cloned from (e.g. a fork, an internal mirror, an SSH remote) and a longer one answers for one remote only.
