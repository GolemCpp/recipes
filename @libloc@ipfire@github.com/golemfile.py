...
def configure(project):
    ...
    task = project.library(name='libloc',
                           targets=['loc'],
                           deps=['openssl'],
                           scripts=[script],
...
def build_gcc(ctx):
    ...
    ret = subprocess.call(['intltoolize', '--force', '--automake'],
                          cwd=repo_dir)
    if ret:
        raise RuntimeError("ERROR: intltoolize")

    ret = subprocess.call(['autoreconf', '--install', '--symlink'],
                          cwd=repo_dir)
    if ret:
    ...
    configure_cflags += [
        '-I{}'.format(ctx.find_dependency_includes('openssl')[0])
    ]
    configure_ldflags += [
        '-L{}'.format(ctx.find_dependency_libraries('openssl')[0])
    ]
